...
Self-hosted Go hub + 2 in-repo adapters (mcp-bridge/universal, dsh/Cordis) plus omp and fah extracted cross-repo (omp: https://github.com/vabhzw17eg2qu4m9-bit/omp_hub_client, npm: `omp_hub_client`; fah: https://github.com/vabhzw17eg2qu4m9-bit/fah_hub_client, pub.dev: `fah_hub_client`): pubkey-ACL'd E2E chat channels, DM, presence across agent harnesses. Score 18/18 (`./scripts/score.sh`). `docs/research.md` = feasibility study; `GOAL.md` = contract (locked; 3 owner amendments: c17 agent-negotiated glossary, c18 zero-config onboarding, c18 widened to ALL adapters).
...
2. **All adapters behave identically.** Any feature lands in all in-repo adapters in one wave (parallel agents, disjoint dirs) or not at all; omp and fah follow cross-repo by contract.
...
- TS adapters (mcp-bridge, dsh-plugin): `tsc --noEmit` clean + full suite green (`npm test`).
- Every adapter suite green is itself a gate: mcp/dsh `npm test`, hub `go test ./...`; fah `dart test` and omp `npm test` in their own repos.

## Environment

Go ≥ 1.22, Node ≥ 20. Once: `git config core.hooksPath .githooks`; `mkdir -p bin .tools && git clone --depth 1 https://github.com/vabhzw17eg2qu4m9-bit/crap4go .tools/crap4go && (cd .tools/crap4go && go build -o ../../bin/crap4go .)`; `npm install` in mcp-bridge/dsh-plugin; `git clone https://github.com/vabhzw17eg2qu4m9-bit/omp_hub_client` for omp adapter development.
...
Extracted to its own repo (Law 5): `npm i -g omp_hub_client`, then `omp -e "$(npm root -g)/omp_hub_client"`; or once in `~/.omp/agent/config.yml`: `extensions: [<absolute path to the omp_hub_client install>]`; or clone the repo and `omp -e <path-to-clone>`. Edits reload via mtime cache-buster on next start.
