...
export function buildServer(dap: DapClient): McpServer {
  ...
  server.registerTool('dap_connect', {
    title: 'DAP connect',
    description: 'Connect to any DAP hub at runtime (a manual invitation): host (hub.example.com, hub:8787, or ws(s)://… — ws:// and /ws are assumed when omitted), optional name (display name AND identity: a new name means a name-derived key file and a new agentId; same name = same agent everywhere), optional channel (default room, joined after connect and on every later launch; persisted to the config file). First connect to a hub that requires auth: set DAP_MASTER_SECRET to enroll once — the issued client secret is then persisted to the config file and reused automatically — or set DAP_CLIENT_SECRET / a config clientSecret to connect without enrolling. NOTE: if that room already exists on the target hub under another member\'s key, ask a member to dap_invite you — a blind join lets you post but members cannot read you.',
    inputSchema: {
